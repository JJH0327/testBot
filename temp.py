...
tp = 0.03
sl = 0.015
volume = 100 # volume for one order (if its 10 and leverage is 10, then you put 1 usdt to one position)
leverage = 10
...
def bolinger(symbol):
    kl = klines(symbol)
    bol = ta.volatility.BollingerBands(kl.Close)
    if kl.Close.iloc[-1] > bol.bollinger_hband().iloc[-1]:
        return 'up'
    if kl.Close.iloc[-1] < bol.bollinger_lband().iloc[-1]:
        return 'down'
    else:
        return 'none'
