...
tp = 0.012
sl = 0.006
volume = 1000  # volume for one order (if its 10 and leverage is 10, then you put 1 usdt to one position)
leverage = 10
...
def ema200_50(symbol):
    ...
    if ema50.iloc[-3] > ema200.iloc[-3] and ema50.iloc[-2] > ema200.iloc[-2] and ema50.iloc[-1] < ema200.iloc[-1]:
        ...
        return 'none'
def bolinger_signal(symbol):
    kl = klines(symbol)
    bolinger = ta.volatility.BollingerBands(kl.Close)
    if kl.Close.iloc[-1] > bolinger.bollinger_hband().iloc[-1]:
        return 'up'
    if kl.Close.iloc[-1] < bolinger.bollinger_lband().iloc[-1]:
        return 'down'
    else:
        return 'none'
    ...
    if balance != None:
        ...
        if len(pos) < qty:
            for elem in symbols:
                ...
                sleep(1)
                signal1 = str_signal(elem)
                # signal = rsi_signal(elem)
                signal = ema200_50(elem)
                print(elem,"",signal," ",signal1)
                ...
                if signal == 'up' and signal1 == 'up' and elem != 'USDCUSDT' and not elem in pos and not elem in ord and elem != symbol:
                    ...
                    sleep(10)
                    # break
                if signal == 'down' and signal1 == 'down' and elem != 'USDCUSDT' and not elem in pos and not elem in ord and elem != symbol:
